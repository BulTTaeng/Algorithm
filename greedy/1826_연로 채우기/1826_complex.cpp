...
bool solve(){
    ...
    while(1){
        ...
        if(station.empty()){ // if we don't have source of fuel anymore.
            while(!fuel.empty()){
                currloc += fuel.top();
                ans++;
                if(currloc >= L){
        ...
        // if we have station left

        if(fuel.empty()){ // if we don't have any fuel left
            return false;
        }
        
        while(!fuel.empty() && currloc < station.top().first){ // visit station to get the fuel to reach next station.
            currloc += fuel.top();
            ans++;
            fuel.pop();
        }
